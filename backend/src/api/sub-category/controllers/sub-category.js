...
module.exports = createCoreController("api::sub-category.sub-category", ({ strapi }) => ({
  ...
  async createSubCategory(ctx) {
    try {
      ...
      const entry = await strapi.entityService.create("api::sub-category.sub-category", {
        data: {
  ...
  async update(ctx) {
    try {
      ...
      const valid = await validedData(data)
      if (valid.err) return ctx.badRequest(valid.err);
      if (ctx.request.files["file"]) { await deleteSubCategoryFile(id) }
      const entry = await strapi.entityService.update("api::sub-category.sub-category", id, {
        data: {
          category_id: data.category_id,
          sub_category_name: data.sub_category_name,
          display_date: data.display_date,
