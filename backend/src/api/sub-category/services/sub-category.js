...
module.exports = {
  ...
  async validedData(data) {
    try {
      if (!data.sub_category_name) {
        return { err: "sub_category_name is required" };
      }
      if (!data.display_date) {
        return { err: "display_date is required" };
      }
      if (!data.status) {
        return { err: "status is required" };
      }
      if (!data.move) {
        return { err: "move is required" };
      }
      if (!data.url) {
        return { err: "url is required" };
      }
      if (!data.is_new) {
        return { err: "is_new is required" };
      }
      if (!data.slider) {
        return { err: "slider is required" };
      }
  ...
  async updateImageUrl(id) {
    try {
      let findUploadData = await strapi.entityService.findOne(
