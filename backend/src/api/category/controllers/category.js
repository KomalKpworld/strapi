...
const {
  subCategoryProductRecordDelete,
  updateImageUrl,
  deleteCategoryFile,
...
module.exports = createCoreController(
  ...
  ({ strapi }) => ({
    ...
    async deleteCategory(ctx) {
      try {
        const id = ctx.request.params.id;
        let productAndSubCategoryRecordDelete =
          await subCategoryProductRecordDelete(id);
        if (productAndSubCategoryRecordDelete.err) {
          return ctx.send(productAndSubCategoryRecordDelete.err);
        }
        let fileDelete = await deleteCategoryFile(id);
        if (fileDelete.err) {
          return ctx.send(fileDelete.err);
        }
