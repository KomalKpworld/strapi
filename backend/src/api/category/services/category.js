...
module.exports = {
    ...
    try {
      ...
      for (let i = 0; i < result.rows.length; i++) {
        ...
        if (result5.rows.length !== 0) {
          ...
          let productFileDeleteResult = await strapi.db.connection.raw(
            productFileDelete
          );
          let productFolderyFileDeleteResult = await strapi.db.connection.raw(
            productFileFolderDataDelete
          );
          let productFileRelatedDeleteResult = await strapi.db.connection.raw(
            productFileRealetedDataDelete
          );
        }
        ...
        if (result4.rows.length !== 0) {
          ...
          let subCategoriesFileDeleteResult = await strapi.db.connection.raw(
            subCategoriesFileDelete
          );
          let subCategoriesFolderyFileDeleteResult =
            await strapi.db.connection.raw(subCategoriesFileFolderDataDelete);
          let subCategoriesFileRelatedDeleteResult =
            await strapi.db.connection.raw(subCategoriesFileRealetedDataDelete);
        }
  ...
  async deleteCategoryFile(id) {
    try {
      ...
      if (result4.rows.length !== 0) {
        ...
        let categoryFileDeleteResult = await strapi.db.connection.raw(
          caegoryFileDelete
        );
        let categorFolderyFileDeleteResult = await strapi.db.connection.raw(
          caegoryFileFolderDataDelete
        );
        let categoryFileRelatedDeleteResult = await strapi.db.connection.raw(
          caegoryFileRealetedDataDelete
        );
      }
      return { msg: "record delete successfully" };
    } catch (error) {
  ...
  async updateImageUrl(id) {
    try {
      const findUploadData = await strapi.entityService.findOne(
      ...
      if (findUploadData.file[0]) {
        ...
        updateEtntry = await strapi.entityService.update(
          "api::category.category",
          id,
          {
            data: {
